<template>
  ...
  export default defineComponent({
    setup() {
      ...
      const dbRequest = window.indexedDB.open('conductorDatabase', 1);
      ...
      const addConductorToDB = (fullName: string, phoneNumber: string, cpf: string, gmail: string): void => {
        if (!db) return;
        const transaction = db.transaction(['conductors'], 'readwrite');
        const objectStore = transaction.objectStore('conductors');
        objectStore.add({ fullName, phoneNumber, cpf, gmail });
        fetchConductorsFromDB();
      };
      ...
      // Fetch conductors from IndexedDB when the component is mounted
